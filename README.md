...
Graphēon now includes the first passive agent slice end to end: agent registry records, enrollment keys, approval workflow, low-impact policy profiles, an outbound-only check-in API, an admin fleet management view at `/agents`, and a lightweight host-side runtime with `systemd` service/timer units. Agent ingest stores evidence-backed host identity and topology facts so host detail and the Network Map can show source, observer, confidence, timestamps, attachment, routing, naming, DHCP, DNS, flow, L2, and segment evidence without treating every passive observation as verified identity. The runtime can optionally filter host-local network noise such as loopback, link-local addresses, and local virtualization bridge interfaces, forward bounded normalized topology evidence from collector output files, and run an explicitly requested bounded passive tcpdump observation window that is parsed locally and deleted without raw pcap upload. Network maps use observed or configured CIDR boundaries when available and leave unknown networks unresolved instead of guessing a fixed subnet. The current bootstrap model uses admin-created enrollment keys and one per-agent API key after approval.
